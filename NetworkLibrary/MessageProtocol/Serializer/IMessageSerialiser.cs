...
namespace MessageProtocol
{
        ...
        where E : IMessageEnvelope, new()
    {
        ...
        void EnvelopeMessageWithBytes(Stream serialisationStream, E empyEnvelope, byte[] payloadBuffer, int offset, int count);
        void EnvelopeMessageWithInnerMessage<T>(Stream serialisationStream, E empyEnvelope, T payload);
        byte[] Serialize<T>(T record);
